...
const DecreaseAccessoryForm = () => {
    const {AllAccessories,DecreaseQuantityResponse,DecreaseQuantityRequest} = AccessoriesStore();
    ...
    const handleSubmit = async (e) => {
        ...
        await DecreaseQuantityRequest(localStorage.getItem("TOKEN"), selectedAccessoryID, quantity);
    ...
    useEffect(() => {
        if (hasSubmitted && DecreaseQuantityResponse.status === "Success") {
            alert(DecreaseQuantityResponse.message);
            setHasSubmitted(false);
            setSelectAccessoryID("");
            setQuantity("");
        }
        if (hasSubmitted && DecreaseQuantityResponse.status === "Error") {
            alert(DecreaseQuantityResponse.message);
            setHasSubmitted(false);
        }
    }   
    ...
    return (
        <div>
             <form onSubmit={handleSubmit} className="max-w-md mx-auto p-6 shadow-md rounded-md space-y-4">
                <h2 className="text-2xl font-semibold text-[#FD4075] text-center mb-5">Decrease Accessory</h2>
...
export default DecreaseAccessoryForm;